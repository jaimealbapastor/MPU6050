...
class CustomTableModel(QAbstractTableModel):
    ...
    def appendRow(self, row_data):
        ...
        if isinstance(row_data, pd.DataFrame):
            correct_format = True
            self.input_data = pd.concat([self.input_data, row_data], ignore_index=True)
            ...
            if len(row_data) == self.column_count:
                ...
                new_row["Ax"] = row_data[1]
                new_row["Ay"] = row_data[2]
                new_row["Az"] = row_data[3]
                # self.input_data = self.input_data.append(new_row, ignore_index=True)
            ...
            print(
                f"Invalid row data format. Should be pd.DataFrame or list but is {type(row_data)}"
            )
